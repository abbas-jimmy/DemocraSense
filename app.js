...
const css = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-primary:hover { background: var(--gold-l); transform: translateY(-2px); box-shadow: 0 8px 20px rgba(255,153,51,0.35); }
  .btn-outline {
  ...
  .ballot-option.selected {
    background: #e8f5e9; border-color: var(--sage);
  ...
  .eligibility-result.eligible { background: rgba(19,136,8,0.2); border: 1px solid rgba(19,136,8,0.5); color: #6fcf6f; }
  .eligibility-result.not-eligible { background: rgba(192,57,43,0.2); border: 1px solid rgba(192,57,43,0.5); color: #f48; }
  .eligibility-result.error { background: rgba(255,153,51,0.15); border: 1px solid rgba(255,153,51,0.4); color: var(--gold-l); }
  ...
  .req-req { background: rgba(19,136,8,0.3); color: #6fcf6f; }
  .req-no { background: rgba(192,57,43,0.3); color: #f87; }
  .note-box {
    margin-top: 1.5rem; padding: 1rem 1.2rem;
    background: rgba(255,153,51,0.1); border-left: 3px solid var(--gold);
  ...
  .timeline-tag {
    ...
    background: rgba(255,153,51,0.1); color: var(--gold);
  ...
  .user-msg .msg-avatar { background: rgba(19,136,8,0.2); border-color: rgba(19,136,8,0.4); }
  .msg-bubble {
    background: rgba(255,255,255,0.08);
    border: 1px solid rgba(255,255,255,0.1);
  ...
  .user-msg .msg-bubble {
    background: rgba(19,136,8,0.2); border-color: rgba(19,136,8,0.3);
  }
  .quick-questions {
  ...
  .quick-questions button:hover { background: rgba(255,153,51,0.25); }
  .chat-input-area {
...
function checkEligibility() {
  ...
  if (age >= 18) {
    ...
    result.innerHTML = `
      ...
      <br>
      <p>🗳 <strong>Next Step:</strong> Register to vote at <a href="https://voters.eci.gov.in" target="_blank" style="color:var(--gold-l);text-decoration:underline;">voters.eci.gov.in</a> if you haven't already. Check if your name is on the electoral roll!</p>
    `;
  } else if (age >= 17) {
    ...
    result.innerHTML = `
      ...
      <br>
      <p>💡 <strong>Tip:</strong> You can pre-register on the electoral rolls before turning 18 if the qualifying date (Jan 1) falls within your 18th year. Check at <a href="https://voters.eci.gov.in" target="_blank" style="color:var(--gold-l);text-decoration:underline;">voters.eci.gov.in</a>.</p>
      <p>📋 You can submit <strong>Form 6</strong> in advance so you're on the electoral roll the moment you qualify!</p>
    `;
  } else {
    ...
    result.innerHTML = `
      ...
      <br>
      <p>🎓 <strong>In the meantime:</strong> Keep learning about civics, follow elections on ECI's website, and talk to eligible voters in your community. The best voters are informed voters!</p>
      <p>📋 Many states allow <strong>pre-registration via Form 6</strong> so you're ready to vote when you turn 18!</p>
    `;
  }
}
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        system: `You are DemocraSense AI, a friendly, nonpartisan, and educational assistant focused entirely on helping people understand the democratic process, elections, voting, and civic participation in India.
...
- Keep answers educational and factual, citing reputable Indian civic sources when helpful (voters.eci.gov.in, eci.gov.in, myneta.info, prsindia.org, adrindia.org, etc.)
- If asked about specific candidates or parties beyond factual descriptions, politely redirect to resources that let the user make their own informed decision (like MyNeta.info, ADR India, etc.)
...
- If asked about something unrelated to voting/elections/democracy in India, politely redirect: "I'm specialized in helping with Indian voting and democracy topics. Let me know if you have any election-related questions!"`,
